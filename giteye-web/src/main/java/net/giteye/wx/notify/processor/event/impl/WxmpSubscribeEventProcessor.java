 ...
 */
@Component
public class WxmpSubscribeEventProcessor implements WxmpEventProcessor {
    ...
    public String process(Map<String, Object> body) {
        log.info("进入到订阅事件类处理器");
        String openId = body.get("FromUserName").toString();
        String qrscene = null;
        if (body.containsKey("EventKey")) {
            String eventKey = body.get("EventKey").toString();
            qrscene = eventKey.split("_")[1];
        }
    ...
    public WxmpEventEnum getWxmpEventEnum() {
        return WxmpEventEnum.SUBSCRIBE;
    }
